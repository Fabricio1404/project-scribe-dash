...
export default function Dashboard() {
  const enEjecucion = proyectos.filter((p) => p.estado === "En ejecuci\xF3n");
  ...
  >
        ...
        <Panel>
          ...
          <ul className="space-y-3">
            {alertas.map((a) => <li key={a.id} className="rounded-md border border-border p-3">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <div>
          ...
          <DataTable head={["L\xEDnea", "Director actual", "Proyectos", "Estado"]}>
        ...
        <div className="grid gap-4 sm:grid-cols-3 xl:grid-cols-1">
          <StatCard label="Publicaciones" value={publicaciones.length} detail={`${publicaciones.filter((p) => p.visibilidad === "P\xFAblica").length} p\xFAblicas`} />
          <StatCard
  ...
  />
    ...
    detail={`${convenios.filter((c) => c.estado === "En negociaci\xF3n").length} en negociaci\xF3n`}
  />
        </div>
      </div>
    </AppShell>;

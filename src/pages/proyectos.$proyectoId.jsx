...
export default function ProyectoDetalle() {
  ...
  if (!p) return <ProyectoNoEncontrado />;
  ...
  />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Código" value={p.codigo} detail={getLinea(p.lineaId)?.nombre ?? "Sin l\xEDnea"} />
        ...
        <StatCard label="Financiador" value={p.financiador} detail={`Direcci\xF3n: ${getInvestigador(p.directorId)?.nombre ?? "sin asignar"}`} />
      </div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
        <Panel>
          ...
          <ul className="space-y-4">
            {p.objetivos.map((o) => <li key={o.descripcion}>
    ...
    return <li key={m.investigadorId} className="flex items-center justify-between gap-3 text-sm">
                  <span>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <div>
          ...
          <DataTable head={["Actividad", "A\xF1o", "Estado"]}>
            {p.cronograma.map((a) => <tr key={a.nombre}>
        ...
        <div>
          ...
          <DataTable head={["Rubro", "Categor\xEDa", "Asignado", "Ejecutado"]}>
            {p.presupuesto.map((r) => <tr key={r.rubro + r.anio}>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        <Panel>
          ...
          <ul className="space-y-3">
            {p.hitos.map((h) => <li key={h.fecha + h.texto} className="rounded-md border border-border p-3 text-sm">
            ...
            {p.contingencias.map((c) => <li key={c.fecha + c.texto} className="rounded-md border border-border p-3 text-sm">
            ...
            {p.hitos.length + p.contingencias.length === 0 ? <li className="text-sm text-muted-foreground">Sin registros.</li> : null}
          </ul>
        </Panel>
        ...
        <Panel>
          ...
          <ul className="mt-2 space-y-2 text-sm">
            {pubs.length ? pubs.map((pub) => <li key={pub.id}>
                  <span className="block">{pub.titulo}</span>
                  <span className="block text-xs text-muted-foreground">
                    {pub.tipo} · {pub.revista} · {formatDate(pub.fecha)}
                  </span>
                </li>) : <li className="text-muted-foreground">Sin publicaciones registradas.</li>}
          </ul>

          <p className="mt-4 text-xs font-semibold uppercase tracking-wide text-muted-foreground">Transferencias</p>
          <ul className="mt-2 space-y-2 text-sm">
            {transf.length ? transf.map((t) => <li key={t.id} className="flex items-center justify-between gap-3">
                  <span>{t.nombre}</span>
                  <Chip tone={estadoTone(t.estado)}>{t.estado}</Chip>
                </li>) : <li className="text-muted-foreground">Sin transferencias asociadas.</li>}
          </ul>
        </Panel>
      </div>
    </AppShell>;
